// ================================================
// TICKR — ASX Trade Registry — Google Apps Script Backend
...
const HEADERS = {
  trades:   ['id','date','code','type','qty','price','amount','commissionPct','commission','total','notes'],
  codes:    ['code','name','currentPrice'],
  types:    ['name','direction'],
  settings: ['key','value']
};
  ...
  return data.slice(1).filter(row => row[0] !== '' && row[0] !== null).map(row => {
    ...
    headers.forEach((h, i) => {
      ...
      if (val === null || val === undefined || val === '') {
        ...
        obj[h] = Number.isInteger(val) ? String(val) : String(Math.round(val*100000)/100000);
      } else {
        obj[h] = String(val);
      }
    });
